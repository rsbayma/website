...
Alguns comandos úteis para definição e simulação de um sistema
...
Definição das matrizes para um sistema hipotético
```{python}
A = np.array([[-2, -1],[1,0]])
B = np.array([[0],[8]])
C = np.array([[0, 2]])
...
Definição do sistema em espaço de estados, usando as matrizes definidas
```{python}
sis = ct.ss(A,B,C,D)
```

Visualização do sistema (muito rudimentar)
```{python}
sis
```


Simulação para condições iniciais
```{python}
x0 = np.array([[10], [-4]])
r = ct.initial_response(sis,T=10, X0=x0)
```

Plotagem da resposta
```{python}
import matplotlib.pyplot as plt
plt.plot(r.t,r.x[0],r.t, r.x[1])
plt.xlabel('Tempo (seg)')
```

Simulação de resposta ao degrau unitário (CI nulas)
```{python}
r= ct.step_response(sis,T=10)
```

Plotagem da resposta (apenas o sinal de saída)
```{python}
...
Obs: isto parece estar bem implementado para sistemas SISO. Para sistemas MIMO pode dar problema.

Podemos melhorar a saída com o SymPy, mas isso exige um pouco mais de codificação que foge do nosso escopo.
...
Obs: quando o sistema não possuir zeros, a matriz retornada é vazia. 
